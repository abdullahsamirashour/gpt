...
if summary_path:
    est = report.get("production_estimate") or {}
    with open(summary_path, "a", encoding="utf-8") as fh:
        fh.write("## Gemini 3.5 Live benchmark\n\n")
        fh.write(f"- Validated chunk: {report.get('validated_chunk_sec')} s\n")
        fh.write(f"- Concurrency: {report.get('concurrency')}\n")
        if est:
            fh.write(f"- Estimated wall clock: {est.get('estimated_wall_clock_min')} min\n")
            fh.write(f"- Ideal audio floor: {est.get('ideal_stream_floor_min')} min\n")
